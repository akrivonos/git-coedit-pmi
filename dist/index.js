 ...
 */
export function coeditPmi(repoRoot, options = {}) {
    const { commitLimit = 500, minCooccur = 2, minPmi = 1, minCommits = 5, extensions = DEFAULT_EXTENSIONS, } = options;
    ...
    const accept = (f) => (extSet ? extSet.has(extname(f).toLowerCase()) : true);
    const fileCount = new Map();
    const pairCount = new Map(); // key: "a\0b" with a < b
    for (const block of commitBlocks) {
        const files = Array.from(new Set(block.split('\n').map((l) => l.trim()).filter((l) => l.length > 0))).filter(accept);
        if (files.length < 2)
            continue;
 ...
 */
export function coeditNeighbors(pairs, file, limit = 10) {
    const out = [];
    for (const p of pairs) {
        if (p.a === file)
            out.push({ file: p.b, pmi: p.pmi, cooccur: p.cooccur });
        else if (p.b === file)
            out.push({ file: p.a, pmi: p.pmi, cooccur: p.cooccur });
    }
